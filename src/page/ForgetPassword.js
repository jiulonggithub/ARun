...
import { forgetPasswordCode,forgetPassword } from '../requests/http'
...
export default class ForgetPassword extends Component{
    ...
    getCode(e){
        ...
        if(this.validMobile(this.state.username) ){
            if(this.state.getCodeFlag){
                ...
                forgetPasswordCode(_data).then((res)=>{
    ...
    TapSubmit(e){
        ...
        if(this.validMobile(this.state.username) && this.validYzm(this.state.code) && this.validPwd(this.state.password) ){
            let _data = {
                phone: self.state.username,
                password: self.state.password,
                code: self.state.code,
            };
            forgetPassword(_data).then((res)=>{
                console.log(res);
                if(parseInt(res.data.errcode)===0){
        ...
        return <View style={styles.loginPage}>
            ...
            <View style={styles.titleNavs} >
                <TouchableOpacity activeOpacity={1}>
                    <Text style={[styles.navText,styles.activeNav]}>忘记密码</Text>
                </TouchableOpacity>
            </View>
            ...
            <View style={{marginTop:20}}>
                <TextInput style={styles.inputText} placeholder="设置密码" onChangeText={(e) => this.passwordChange(e)}  placeholderTextColor="#bbb" underlineColorAndroid='transparent' secureTextEntry={true}/>
            </View>
